...
import { REGIONS, GRAPES, GRAPE_TRAITS, TERMS } from "../data/wineData";
...
function getPools(poolKey) {
  ...
  return {
    ...
    grapes: GRAPES.filter(filter),
    grapeTraits: GRAPE_TRAITS.filter(filter),
    terms: TERMS.filter(filter),
  };
}
...
function buildItem(category, entry, pools, type) {
  ...
  if (category === "grape-trait") {
    const prompt = entry.clue;
    const answer = entry.grape;
    if (type === "text") return { category, prompt, answer };
    const wrongs = shuffle(pools.grapeTraits.filter((e) => e.grape !== entry.grape)).slice(0, 3);
    const options = shuffle([answer, ...wrongs.map((w) => w.grape)]);
    return { category, prompt, answer, options };
  }
  // term
  const prompt = entry.definition;
  const answer = entry.term;
  if (type === "text") return { category, prompt, answer };
  const wrongs = shuffle(pools.terms.filter((e) => e.term !== entry.term)).slice(0, 3);
  const options = shuffle([answer, ...wrongs.map((w) => w.term)]);
  return { category, prompt, answer, options };
}
...
function buildRound(cfg) {
  ...
  const tagged = [
    ...pools.regions.map((e) => ({ category: "region", entry: e })),
    ...pools.grapes.map((e) => ({ category: "grape", entry: e })),
    ...pools.grapeTraits.map((e) => ({ category: "grape-trait", entry: e })),
    ...pools.terms.map((e) => ({ category: "term", entry: e })),
  ];
  const picks = shuffle(tagged).slice(0, ROUND_LENGTH);
  return picks.map((p) => buildItem(p.category, p.entry, pools, cfg.type));
}

const CATEGORY_LABEL = { region: "Vinregion", grape: "Drue", "grape-trait": "Drue", term: "Vinord" };

export default function Vinjakt() {
  usePageTitle("Vinjakt");
  ...
  const shareResult = async () => {
    ...
    const text = `Vinjakt (${DIFFICULTY[difficulty].label}) — ${correctCount}/${ROUND_LENGTH}\n${grid}\nfellesnevner.no/vinjakt`;
  ...
  return (
    <PageShell>
      <div style={shared.header}>
        ...
        <h1 style={shared.title}>Vinjakt</h1>
        <p style={shared.subtitle}>Vinregioner, druer og vinord — {ROUND_LENGTH} spørsmål per runde.</p>
      </div>
      ...
      {status === "done" && (
        <div style={styles.endBanner}>
          ...
          <SaveScoreRow game="vinjakt" difficulty={difficulty} score={correctCount} timeSeconds={finalTime} />
